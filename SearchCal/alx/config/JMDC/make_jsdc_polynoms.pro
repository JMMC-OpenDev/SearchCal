...
;
  ...
  FOR N=0, NCOLORS-1 DO BEGIN
     ...
     PLOTERR,SPTYPE_B[A],RESIDU[A,N],E_RESIDU[A,N],PSYM=8,NOHAT=1,YRANGE=[0,1.3],YSTYLE=1,XTITLE='SPECTRAL TYPE' & OPLOT,SPTYPE_B[A],FIT,COLOR=64000
     RES_B[A,N]=(RESIDU[A,N]-FIT)/E_RESIDU[A,N] & CHI2=MEAN(RES_B[A,N]^2) ; & IF (DOPRINT) THEN PRINT,N,CHI2,N_ELEMENTS(A)
  ENDFOR
  !P.MULTI=0
rep='' & if (dowait) then READ, 'press any key to continue', rep
;
; Histogram of residuals
  BIN=0.02 & HH=HISTOGRAM(RESIDU[GOOD_B,*],BINSIZE=BIN,LOCATIONS=XX) & XX=XX+BIN/2. & IF (DOPRINT) THEN PRINT,' '
  RES=GAUSSFIT(XX,HH,NTERMS=3,Z) & PLOT,XX,HH,PSYM=10 & OPLOT,XX,RES,COLOR=65000 & IF (DOPRINT) THEN PRINT,Z & IF (DOPRINT) THEN PRINT,' '
rep='' & if (dowait) then READ, 'press any key to continue', rep
;
...
;
     FOR II=0, ND-1 DO BEGIN
        KK=II*NCOLORS & ICOV[KK:KK+NCOLORS-1,KK:KK+NCOLORS-1]=INVERT(DCOV_IB[*,*,II],STATUS,/DOUBLE) & IF (STATUS NE 0) THEN IF (DOPRINT) THEN PRINT,STATUS,II
     ENDFOR
;
...
;
     DCOV_OB=DCOV_IB & ERR=DBLARR(NCOLORS,ND)
     FOR II=0, ND-1 DO BEGIN
        FOR JJ=0, NCOLORS-1 DO BEGIN
           FOR KK=0, NCOLORS-1 DO BEGIN
              FOR Z=0, DEG DO BEGIN
                 FOR W=0, DEG DO BEGIN
                    DCOV_OB[KK,JJ,II]=DCOV_OB[KK,JJ,II]+SPTYPE_B[GOOD_B[II]]^Z*SPTYPE_B[GOOD_B[II]]^W*MCOV_POL[(DEG+1)*JJ+Z,(DEG+1)*KK+W]
                    IF (KK EQ JJ) THEN ERR[JJ,II]=ERR[JJ,II]+SPTYPE_B[GOOD_B[II]]^Z*SPTYPE_B[GOOD_B[II]]^W*MCOV_POL[(DEG+1)*JJ+Z,(DEG+1)*KK+W]
                 ENDFOR
              ENDFOR
           ENDFOR
        ENDFOR
        C=DCOV_OB[*,*,II] & B=INVERT(C,/DOUBLE,STATUS) & A=TOTAL(B) & IF (STATUS NE 0) THEN IF (DOPRINT) THEN PRINT,II,STATUS
...
; Plot histograms
  !P.MULTI=[0,3,2]
  FOR N=0, NCOLORS-1 DO BEGIN
     BIN=0.4 & HH=HISTOGRAM(RES_C[GOOD_B,N],BINSIZE=BIN,LOCATIONS=XX) & XX=XX+BIN/2. & RES=GAUSSFIT(XX,HH,NTERMS=3,Z)
     PLOT,XX,HH,PSYM=10,XRANGE=[-5,5],XSTYLE=1 & OPLOT,XX,RES,COLOR=65000 & IF (DOPRINT) THEN PRINT,Z
  ENDFOR
  HH=HISTOGRAM(RES_C[GOOD_B,*],BINSIZE=BIN,LOCATIONS=XX) & XX=XX+BIN/2. & RES=GAUSSFIT(XX,HH,NTERMS=3,Z)
  PLOT,XX,HH,PSYM=10,XRANGE=[-5,5],XSTYLE=1 & OPLOT,XX,RES,COLOR=1000 & IF (DOPRINT) THEN PRINT,Z & PAR=PARAMS & EPAR=E_PARAMS
  !P.MULTI=0
rep='' & if (dowait) then READ, 'press any key to continue', rep

; LBO: CHI2TH
L=WHERE(FINITE(CHI2_MD[GOOD_B]), NL)
IF (DOPRINT) THEN PRINT,"SAMPLES:", N_ELEMENTS(GOOD_B), "N CHI2:",NL

  S=WHERE(CHI2_MD[GOOD_B] LE NSIG^2, NS) & A=MEAN(CHI2_MD[GOOD_B[S]])
  IF (DOPRINT) THEN PRINT,' ' & IF (DOPRINT) THEN PRINT,'NSTAR =',NS,N_ELEMENTS(GOOD_B),'    CHI2_POL =',CHI2_POL,'    CHI2_MD =',A
; Plot results
  !P.MULTI=[0,3,2]
  FOR N=0, NCOLORS-1 DO BEGIN
     PLOTERR,ALOG10(DIAM_I[GOOD_B]),DIAM_B[GOOD_B,N],ED,EDIAM_B[GOOD_B,N],NOHAT=1,PSYM=8 & OPLOT,ZZ,ZZ,COLOR=64000
  ENDFOR
  PLOTERR,ALOG10(DIAM_I[GOOD_B]),DMEAN_B[GOOD_B],ED,EDMEAN_B[GOOD_B],NOHAT=1,PSYM=8
