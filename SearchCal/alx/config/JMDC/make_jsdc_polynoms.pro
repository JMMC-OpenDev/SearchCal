...
;
  IF (MODE EQ 'FIX') THEN GOOD_B=GOOD_FIX ELSE BEGIN
    ...
    IF (LUM_CLASSES EQ 123) THEN GOOD_B=WHERE((LUMCLASS_B[W] EQ 1 OR LUMCLASS_B[W] EQ 2 OR LUMCLASS_B[W] EQ 3) AND DLUMCLASS_B[W] EQ 0 AND SPTYPE_B[W] NE -1 AND DIAM_I[W] NE -1 AND EDIAM_I[W] NE -1 AND FINITE(DIAM_I[W]) AND SNR_DIAM_I[W] GT SNR  AND DSPTYPE_B[W] EQ 0)
...
;           IF (IBAND[LL1] NE JBAND[LL2] AND JBAND[LL1] EQ IBAND[LL2]) THEN DCOV_IB[LL1,LL2,*]=-MAT3[LL2,LL1]*R^2+ED^2
     ...
     FOR N=0, NCOLORS-1 DO BEGIN
        ...
        A=WHERE(ABS(RES_B[GOOD_B,N]) LE LAST_TH, NP) 
        IF (NP NE 0) THEN S[A,N]=1
     ENDFOR
...
; V-K      -0.165912647092855      0.0160043072517905   -0.000131734338283244    4.86645648078421E-07   -6.43695785941918E-10
	...
	OLD_PARAMS=[-0.165912647092855, 0.0160043072517905, -0.000131734338283244, 4.86645648078421E-07, -6.43695785941918E-10]
	PRINT,"OLD_PARAMS:",OLD_PARAMS
	PRINT,"PARAMS:",PARAMS[N,*]

    FIT_OLD=DBLARR(NX) & FOR KK=0, DEG DO FIT_OLD=FIT_OLD+OLD_PARAMS[KK]*SP_X^KK
    FIT_NEW=DBLARR(NX) & FOR KK=0, DEG DO FIT_NEW=FIT_NEW+PARAMS[N,KK]*SP_X^KK

    PLOT,SP_X,FIT_NEW,XTITLE='SPECTRAL TYPE',YRANGE=[0,1]
    OPLOT,SP_X,FIT_OLD,COLOR=64000
    ...
    RES=ABS(RES)
    PRINT,"ABS DIFF POLYNOMS (%): MIN: ",MIN(RES)," MEAN: ",MEAN(RES)," MEDIAN: ",MEDIAN(RES)," MAX: ",MAX(RES)
