...
; 2024.1: test V JHK LMN
; USEDBANDS=[1, 3,4,5, 6,7,8] & IBAND=[1,1,1, 1,1,1] & JBAND=[3,4,5, 6,7,8] & NCOLORS=N_ELEMENTS(IBAND)

; 2024.1: test V JHK LMN G Bp Rp
; IR excess in N band causes troubles for fit (=> filter such non-standard stars FIRST ie before fit on scientific reasons like IR flag):
; USEDBANDS=[1, 3,4,5, 6,7,8, 9,10,11] & IBAND=[1,1,1, 1,1,1, 9, 10, 11] & JBAND=[3,4,5, 6,7,8, 5, 5, 5] & NCOLORS=N_ELEMENTS(IBAND)

; 2024.1: test V JHK LM G Bp Rp
; USEDBANDS=[1, 3,4,5, 6,7, 9,10,11] & IBAND=[1,1,1, 1,1, 9, 10, 11] & JBAND=[3,4,5, 6,7, 5, 5, 5] & NCOLORS=N_ELEMENTS(IBAND)


; 2024.1: test V JHK LMN G
; USEDBANDS=[1, 3,4,5, 6,7,8, 9] & IBAND=[1,1,1, 1,1,1, 9] & JBAND=[3,4,5, 6,7,8, 5] & NCOLORS=N_ELEMENTS(IBAND)

; 2024.1: test V JHK LM G (no N)
; USEDBANDS=[1, 3,4,5, 6,7, 9] & IBAND=[1,1,1, 1,1, 9] & JBAND=[3,4,5, 6,7, 5] & NCOLORS=N_ELEMENTS(IBAND)

; USEDBANDS=[1, 3,4,5, 6, 9] & IBAND=[1,1,1, 1, 9] & JBAND=[3,4,5, 6, 5] & NCOLORS=N_ELEMENTS(IBAND)
; USEDBANDS=[1, 3,4,5, 9] & IBAND=[1,1,1, 9] & JBAND=[3,4,5, 5] & NCOLORS=N_ELEMENTS(IBAND)

; 2024.1: test V JHK N G
; USEDBANDS=[1, 3,4,5, 8, 9] & IBAND=[1,1,1, 1, 9] & JBAND=[3,4,5, 8, 5] & NCOLORS=N_ELEMENTS(IBAND)
...
; 2/ Plot residuals & fit band per band
  ...
  FOR N=0,N_ELEMENTS(RESIDU[0,*])-1 DO BEGIN
     ...
     PLOT,SPTYPE_B[GOOD_B]/4.,RESIDU[GOOD_B,N],PSYM=8,YRANGE=[0,1.1],YSTYLE=1,XRANGE=[0,70],XSTYLE=1,XTITLE="SPECTRAL TYPE",XTICKV=DINDGEN(7)*10,XTICKS=7,XTICKNAME=['O0','B0','A0','F0','G0','K0','M0','M10']
     ...
     OPLOTERR,Y1[A],SPECTRAL_DSB[A,N],E_SPECTRAL_DSB[A,N]
  ENDFOR
  !P.MULTI=0
...
; 3/ Resample residuals & fit with polynom
 ...
 !P.MULTI=[0,1,NCOLORS]
  ...
  FOR N=0, NCOLORS-1 DO BEGIN
     ...
     PLOTERR,SPM[S]/4,RM[S,N],ERM[S,N],PSYM=8,NOHAT=1,CHARS=2,XRANGE=[0,70],XSTYLE=1,YSTYLE=1,YRANGE=[YMIN,YMAX],XTITLE="SPECTRAL TYPE",XTICKV=DINDGEN(7)*10,XTICKS=7,XTICKNAME=['O0','B0','A0','F0','G0','K0','M0','M10']
...
; 4/ Plot output versus input diameter & histogram of residuals (database)
  ...
  Q=WHERE(LUMCLASS_B[GOOD_B] EQ 2 AND DLUMCLASS_B EQ 0,M) & IF (M NE 0) THEN OPLOT,ALOG10(DIAM_I[GOOD_B[Q]]),DMEAN_B[GOOD_B[Q]],PSYM=8,COLOR=blue
  Q=WHERE(LUMCLASS_B[GOOD_B] EQ 3 AND DLUMCLASS_B EQ 0,M) & IF (M NE 0) THEN OPLOT,ALOG10(DIAM_I[GOOD_B[Q]]),DMEAN_B[GOOD_B[Q]],PSYM=8,COLOR=yellow
  Q=WHERE(LUMCLASS_B[GOOD_B] EQ 4 AND DLUMCLASS_B EQ 0,M) & IF (M NE 0) THEN OPLOT,ALOG10(DIAM_I[GOOD_B[Q]]),DMEAN_B[GOOD_B[Q]],PSYM=8,COLOR=violet
  Q=WHERE(LUMCLASS_B[GOOD_B] EQ 5 AND DLUMCLASS_B EQ 0,M) & IF (M NE 0) THEN OPLOT,ALOG10(DIAM_I[GOOD_B[Q]]),DMEAN_B[GOOD_B[Q]],PSYM=8,COLOR=red
;
  USERSYM,COS(X),SIN(X),/FILL
  Q=WHERE(LUMCLASS_B[GOOD_B] EQ 1 OR LUMCLASS_B[GOOD_B] EQ 2 OR LUMCLASS_B[GOOD_B] EQ 3,M) & IF (M NE 0) THEN OPLOT,ALOG10(DIAM_I[GOOD_B[Q]]),DMEAN_B[GOOD_B[Q]],PSYM=8,COLOR=yellow
  Q=WHERE(LUMCLASS_B[GOOD_B] EQ 4 OR LUMCLASS_B[GOOD_B] EQ 5,M) & IF (M NE 0) THEN OPLOT,ALOG10(DIAM_I[GOOD_B[Q]]),DMEAN_B[GOOD_B[Q]],PSYM=8,COLOR=red
...
;ELDI=EDIAM_I[GOOD_B]/DIAM_I[GOOD_B]/LN_10 & ZZ=DINDGEN(100)/10-5 & PLOT,ZZ,ZZ,YRANGE=[-1,2],XRANGE=[-1,2],XSTYLE=1,YSTYLE=1,$
;XTITLE="INPUT DIAMETER", YTITLE="COMPUTED DIAMETER",CHARSIZE=1.5
;OPLOTERR,ALOG10(DIAM_I[GOOD_B]),DMEAN_B[GOOD_B],EDIAM_I[GOOD_B]/DIAM_I[GOOD_B]/LN_10,EDMEAN_B[GOOD_B]
;Q=WHERE((LUMCLASS_B[GOOD_B] EQ 1 OR LUMCLASS_B[GOOD_B] EQ 2 OR LUMCLASS_B[GOOD_B] EQ 3) AND DLUMCLASS_B[GOOD_B] EQ 0)
;OPLOTERR,ALOG10(DIAM_I[GOOD_B[Q]]),DMEAN_B[GOOD_B[Q]],EDIAM_I[GOOD_B[Q]]/DIAM_I[GOOD_B[Q]]/LN_10,EDMEAN_B[GOOD_B[Q]]
