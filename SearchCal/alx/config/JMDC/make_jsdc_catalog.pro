...
; Compute mean modeled diameters & errors
  ...
  FOR LL1=0, NCOLORS-1 DO BEGIN
     ...
     FOR LL2=0, NCOLORS-1 DO BEGIN
        ...
        IF (IBAND[LL1] EQ JBAND[LL2] AND JBAND[LL1] NE IBAND[LL2]) THEN DCOV_OC[LL1,LL2,*]=-MAT3[LL1,LL2]*Q^2
        IF (IBAND[LL1] NE JBAND[LL2] AND JBAND[LL1] EQ IBAND[LL2]) THEN DCOV_OC[LL1,LL2,*]=-MAT3[LL2,LL1]*R^2
     ENDFOR
  ENDFOR
...
; Plot histograms
  ...
  IF (DOPRINT) THEN PRINTF,UNITLOG,''
  IF (DOPRINT) THEN PRINTF,UNITLOG,'LUM_CLASSES ='+strtrim(LUM_CLASSES,2)+', NSTAR='+strtrim(NS,2)+', CHI2_DS ='+strtrim(MEAN(CHI2_DS[GOOD_C]),2)
