...
; Initialize parameters & arrays
  IF (N_ELEMENTS(DATA_B.G) GT 0) THEN BEGIN
MAG_C=[TRANSPOSE(DATA_C.B),TRANSPOSE(DATA_C.V),TRANSPOSE(DATA_C.ICOUS),TRANSPOSE(DATA_C.J),TRANSPOSE(DATA_C.H),TRANSPOSE(DATA_C.K),TRANSPOSE(DATA_C.L),TRANSPOSE(DATA_C.M),TRANSPOSE(DATA_C.N),TRANSPOSE(DATA_C.G),TRANSPOSE(DATA_C.BP),TRANSPOSE(DATA_C.RP)]
  EMAG_C=[TRANSPOSE(DATA_C.E_B),TRANSPOSE(DATA_C.E_V),TRANSPOSE(DATA_C.E_ICOUS),TRANSPOSE(DATA_C.E_J),TRANSPOSE(DATA_C.E_H),TRANSPOSE(DATA_C.E_K),TRANSPOSE(DATA_C.E_L),TRANSPOSE(DATA_C.E_M),TRANSPOSE(DATA_C.E_N),TRANSPOSE(DATA_C.E_G),TRANSPOSE(DATA_C.E_BP),TRANSPOSE(DATA_C.E_RP)]
  ENDIF ELSE BEGIN
...
; 2020: LBO: adjust range of fitted SPTYPE [8 - 68]
  IF (LUM_CLASSES EQ 0) THEN GOOD_C=WHERE(SPTYPE_C[W] NE -1 and sptype_c[w]/4. GE 8 AND sptype_c[w]/4. LE 68)
...
; normalize JHK error on max JHK error.
  FOR N=0,NSTAR_C-1 do EMAG_C[N,3:5]=MAX(EMAG_C[N,3:5])
